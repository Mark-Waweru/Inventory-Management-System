# BigShelf Inventory Management System
---

The Inventory Management System is a web-based application built with Ruby on Rails and Hotwire to help administrators manage products and track stock levels in a retail store.<br>
The system provides real-time stock updates, low-stock notifications, and an intuitive user interface for managing inventory efficiently.
---
## Documentation
---

For detailed documentation including architecture diagrams, API specifications, and feature descriptions, please refer to our comprehensive project documentation:
[BigShelf IMS Full Project Documentation](https://docs.google.com/document/d/1Hj64jM0tDR5fhXg-obzKzuKHLz3NmIYt-9tSXjHcFMM/edit?usp=sharing)
---
## Getting Started (Step-by-Step)
Follow this steps to run the Application Locally.

**Step 1: Clone the Repository**
```bash
git clone https://github.com/your-username/your-repo-name.git
cd your-repo-name
```
**Step 2: Install Ruby Dependancies**<br>
Install all required gems using bundler
```bash
bundle install
```
**Step 3: Setup the Database**<br>
Create and migrate the database
```bash
bin/rails db:create
bin/rails db:migrate
bin/rails db:seed
```
**Step 4: Start the Rails Server**<br>
Run the development server
```bash
bin/rails server
```
Open your browser and visit http://localhost:3000. You should see the Rails Application Running locally.<br>
Use this credentials to log In to the Application<br>
Email
```bash
test@gmail.com
```
Password
```bash
password
```